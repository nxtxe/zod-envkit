...
Версия **1.2.0** закрепляет `zod-envkit` как инструмент контрактов, а не просто валидатор.

Добавлено и усилено:

- стабильный public API в рамках 1.x
- контрактное поведение CLI (фиксированные exit-коды)
- расширенные preprod / robustness тесты
- улучшенная согласованность `init`, `generate`, `check`
- обновлённая документация как часть продукта

Начиная с 1.2.0, `zod-envkit` можно воспринимать как **стабильный инструмент для production-использования**.
...
Ветка **1.x** считается стабильной (LTS‑линейкой).  
Breaking‑изменения возможны только при переходе на 2.0.0.
