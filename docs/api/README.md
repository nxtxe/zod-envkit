**zod-envkit**

***

# zod-envkit

## Type Aliases

- [DocsFormat](type-aliases/DocsFormat.md)
- [EnvCheckResult](type-aliases/EnvCheckResult.md)
- [EnvMeta](type-aliases/EnvMeta.md)
- [EnvMetaEntry](type-aliases/EnvMetaEntry.md)
- [GenerateDocsOptions](type-aliases/GenerateDocsOptions.md)
- [LoadEnvFail](type-aliases/LoadEnvFail.md)
- [LoadEnvOk](type-aliases/LoadEnvOk.md)
- [LoadEnvOptions](type-aliases/LoadEnvOptions.md)
- [SortMode](type-aliases/SortMode.md)

## Functions

- [checkEnv](functions/checkEnv.md)
- [formatZodError](functions/formatZodError.md)
- [generateEnvDocs](functions/generateEnvDocs.md)
- [generateEnvExample](functions/generateEnvExample.md)
- [getMissingEnv](functions/getMissingEnv.md)
- [getUnknownEnv](functions/getUnknownEnv.md)
- [isSecretKey](functions/isSecretKey.md)
- [loadEnv](functions/loadEnv.md)
- [mustLoadEnv](functions/mustLoadEnv.md)
- [sortMetaEntries](functions/sortMetaEntries.md)
