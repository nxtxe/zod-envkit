...
Проверка пустых значений смотрит только на ключи из dotenv — обычный `check` без `--production` не падает на whitespace-only значения в dotenv.
...
Используйте в production-пайплайнах, когда нужны дополнительные guardrails поверх обычного `check`, без явного `--strict`. Дополнительные production-правила (placeholder-значения) появятся в следующих релизах.
