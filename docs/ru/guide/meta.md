...
По умолчанию CLI ищет файлы (в таком порядке):

* `./env.meta.json`
* `./examples/env.meta.json`

Если указан другой конфиг (например `-c my.env.json`), также проверяются `./examples/<config>` и `./examples/env.meta.json`.
...
* `deprecated?: boolean | string` — пометить как устаревшее: `true` для символа ⚠️ или строка (например `"Используйте FOO"`) выводится в сгенерированной документации
